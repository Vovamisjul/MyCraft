...
Arm::Arm(string path, const GLchar* vertexPath, const GLchar* fragmentPath) : CommonPlayer::CommonPlayer(path, vertexPath, fragmentPath)
{

}

void Arm::Draw(NetPlayer *otherplayer, Player *player)
{
	...
	if (otherplayer->timeHitting > 0.0f)
	{
		if (otherplayer->isRight)
			model = glm::rotate(model, PI + (MAXTIMEHITTING - otherplayer->timeHitting), up);
		else
			model = glm::rotate(model, PI, up);
	}
	else
