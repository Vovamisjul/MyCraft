...
WorkBench::WorkBench(const GLchar* vertexPath, const GLchar* fragmentPath) : Shader::Shader(vertexPath, fragmentPath)
{
	initialize();
}
...
void WorkBench::initialize()
{
	...
	unsigned char* image = SOIL_load_image("textures/interface/workbench.png", &width, &height, 0, SOIL_LOAD_RGB);
...
void WorkBench::initializeInvNum()
{
	for (int j = 0; j < 2; j++)
	{
		for (int i = 0; i < 24; i++)
		{
			...
			glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBOinvnum[j][i]);
			glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(indices), indices, GL_STATIC_DRAW);
...
void WorkBench::initializeInvCraft()
{
	for (int i = 0; i < 10; i++)
	{
		glGenVertexArrays(1, &VAOinvcraft[i]);
		glGenBuffers(1, &VBOinvcraft[i]);
		glGenBuffers(1, &EBOinvcraft[i]);
		// Bind the Vertex Array Object first, then bind and set vertex buffer(s) and attribute pointer(s).
		glBindVertexArray(VAOinvcraft[i]);

		glBindBuffer(GL_ARRAY_BUFFER, VBOinvcraft[i]);
		glBufferData(GL_ARRAY_BUFFER, sizeof(craftVerticesInv), craftVerticesInv, GL_STATIC_DRAW);

		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBOinvcraft[i]);
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(indices), indices, GL_STATIC_DRAW);
		...
		if (i + 1 < 9)
		{
			if (((i + 1) % 3) != 0)
			{
				craftVerticesInv[0] += 0.10625f;
				craftVerticesInv[5] += 0.10625f;
				craftVerticesInv[10] += 0.10625f;
				craftVerticesInv[15] += 0.10625f;
			}
			else
			{
				craftVerticesInv[0] = craftVerticesInv[5] = -0.35f;
				craftVerticesInv[10] = craftVerticesInv[15] = -0.48f;
				craftVerticesInv[1] -= 0.188889f;
				craftVerticesInv[6] -= 0.188889f;
				craftVerticesInv[11] -= 0.188889f;
				craftVerticesInv[16] -= 0.188889f;
			}
		}
		else
		{
			craftVerticesInv[0] = craftVerticesInv[5] = -0.03f + 0.1125f;
			craftVerticesInv[10] = craftVerticesInv[15] = -0.16f + 0.1125f;
			craftVerticesInv[1] = craftVerticesInv[16] = 0.77f - 0.188889f;
			craftVerticesInv[6] = craftVerticesInv[11] = 0.58f - 0.188889f;
		}
	}
}
...
void WorkBench::drawInv(GLuint *texturesInv, GLuint *texturesNum, Player *player)
{
	...
	for (int i = 0; i < 24; i++)
	{
		if (player->slots[i].num)
		{
			...
			glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);
			glBindVertexArray(0);
			drawInvNum(texturesNum, player);
		}
	}
	for (int i = 0; i < 10; i++)
	{
		if (player->benchCraftSlots[i].num)
		{
			Use();
			glActiveTexture(GL_TEXTURE0);
			glBindTexture(GL_TEXTURE_2D, texturesInv[player->benchCraftSlots[i].id]);
			glBindVertexArray(VAOinvcraft[i]);
			glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);
			glBindVertexArray(0);
			drawInvNum(texturesNum, player);
...
void WorkBench::drawInvNum(GLuint *texturesNum, Player *player)
{
	glDepthFunc(GL_LEQUAL); // set depth f*
	for (int i = 0; i < 24; i++)
	{
